...
from geofree.modifications.style_transfer import StyleTransfer
from geofree.modifications.segmentation import Segmentation
...
if __name__ == "__main__":
    ...
    style_img = Image.open("C:/Users/USER/Desktop/Germany/TUM/Semester1/ML3D/Project/geometry-free-view-synthesis/scripts/style.jpg")
    style_img = style_img.resize((368, 208))
    ...
    while True:
        ...
        if CHANGED:
            with torch.no_grad():
                seg_map = seg.get_segmentaion_map(frame)
                seg_map = torch.tensor(seg_map).unsqueeze(0)

            print(f"Selected object: {selected_object}")
            if selected_object is not None:
                content = torch.tensor(frame/255, dtype=torch.float32).clone().detach().permute(2,0,1).unsqueeze(0)
                style = transforms.ToTensor()(style_img).unsqueeze(0)
                print(f"Style shape: {style.shape}", f"Content shape: {content.shape}", f"Segmentation shape: {seg_map.shape}")
                styled_frame,_ = styler.train(content, style, seg_map.unsqueeze(0), selected_object, num_steps=50)
                
                styled_frame, _ = to_surface(styled_frame)
                surface.blit(styled_frame, (2*w,0))
            CHANGED = False

        if renderer.step is not None:
            print(f"Step: {renderer.step}/{renderer.current_sample.shape[1]}")
            CHANGED = True if renderer.step+1    == renderer.current_sample.shape[1] else False
            RENDERING = False if renderer.step+1 == renderer.current_sample.shape[1] else True

        

        decoded_seg = seg.decode_segmap(seg_map[0].cpu().numpy())

        # Display segmentation  
        seg_image, _ = to_surface(decoded_seg)
        surface.blit(seg_image, (w,0))
        ...
        # If it's the first time ask the user to choose the object
        if step == 0:
            # But chaneg teh order channels
            x, y = seg.get_clicked_pixel(frame/255)
            selected_object = seg_map[0][y][x]
            print(f"Selected object: {selected_object}")
        step +=1
